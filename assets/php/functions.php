...
function followUser($user_id){
    ...
    createNotification($cu['id'],$user_id,"te comenzó a seguir.");
    return mysqli_query($db,$query);
...
function blockUser($blocked_user_id){
    ...
    createNotification($cu['id'],$blocked_user_id,"te bloqueó.");
...
function unblockUser($user_id){
    ...
    createNotification($current_user,$user_id,"te desbloqueó.");
    return mysqli_query($db,$query);   
}
...
function like($post_id){
   ...
   if($poster_id!=$current_user){
    createNotification($current_user,$poster_id,"le dio me gusta a tu publicación.",$post_id);
   }
...
function addComment($post_id,$comment){
 ...
 $comment = mysqli_real_escape_string($db,$comment);
    ...
    if($poster_id!=$current_user){
        createNotification($current_user,$poster_id,"comentó tu publicación.",$post_id);
    }
...
function unlike($post_id){
    ...
    $poster_id = getPosterId($post_id);
    if($poster_id!=$current_user){
        createNotification($current_user,$poster_id,"le sacó el me gusta a tu publicación.",$post_id);
    }
...
function unfollowUser($user_id){
    ...
    createNotification($current_user,$user_id,"te dejó de seguir.");
    return mysqli_query($db,$query);
...
$response['status']=true;
    ...
    if(!$form_data['password']){
        $response['msg']="Contraseña no especificada.";
        $response['status']=false;
        $response['field']='password';
    }
   
    if(!$form_data['username']){
        $response['msg']="Nombre de usuario no especificado.";
        $response['status']=false;
        $response['field']='username';
    }
    
    if(!$form_data['email']){
        $response['msg']="Email no especificado.";
        $response['status']=false;
        $response['field']='email';
    }
    
    if(!$form_data['last_name']){
        $response['msg']="Apellido no especificado.";
        $response['status']=false;
        $response['field']='last_name';
    }
    if(!$form_data['first_name']){
        $response['msg']="Nombre no especificado.";
        $response['status']=false;
        $response['field']='first_name';
    }
    if(isEmailRegistered($form_data['email'])){
        $response['msg']="El e-mail ya se encuentra registrado.";
        $response['status']=false;
        $response['field']='email';
    }
    if(isUsernameRegistered($form_data['username'])){
        $response['msg']="El nombre de usuario ya se encuentra registrado.";
        $response['status']=false;
        $response['field']='username';
    }
...
function validateLoginForm($form_data){
    ...
    $blank=false;
        ...
        if(!$form_data['username_email']){
            $response['msg']="Correo/Nombre de usuario no especificado.";
            $response['status']=false;
        ...
        if(!$blank && !checkUser($form_data)['status'] ){
            $response['msg']="Algo está incorrecto, inténtalo nuevamente.";
            $response['status']=false;
...
function validateUpdateForm($form_data,$image_data){
    ...
    $response['status']=true;
        ...
        if(isUsernameRegisteredByOther($form_data['username'])){
            $response['msg']=$form_data['username']." ya existe.";
            $response['status']=false;
            $response['field']='username';
        }
       ...
       if($image_data['name']){
           ...
           if($type!='jpg' && $type!='jpeg' && $type!='png'){
            $response['msg']="Solo imagenes jpg, jpeg y png están permitidas.";
            $response['status']=false;
            $response['field']='profile_pic';
        }

        if($size>1000){
            $response['msg']="Sube una imagen de menos de 1mb.";
            $response['status']=false;
            $response['field']='profile_pic';
        }
       }
...
function validatePostImage($image_data){
    ...
    $response['status']=true;
        ...
        if(!$image_data['name']){
            $response['msg']="Ninguna imagen está seleccionada.";   // SACAR POSIBLEMENTE -------------------------------------------------
            $response['status']=false;
       ...
       if($image_data['name']){
        ...
        if($size>2000){ 
            $response['msg']="Sube una imagen de menos de 1mb."; // CAMBIAR TAMAÑO POSIBLEMENTE ------------------------
            $response['status']=false;
            $response['field']='post_img';
        }
       }
