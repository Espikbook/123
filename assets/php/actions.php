...
if($response['status']){
    ...
    header('location:../../?login&newuser');
        ...
        echo "<script>alert('Algo está incorrecto.')</script>";
    }
...
if(isset($_GET['login'])){
    ...
    if($response['status']){
     ...
     sendCode($response['user']['email'],'Verifica tu correo',$code);
     }
    ...
    if(isset($_GET['resend_code'])){
            ...
            $_SESSION['code']=$code = rand(111111,999999);
            sendCode($_SESSION['userdata']['email'],'Verifica tu correo',$code);
    ...
    if(isset($_GET['verify_email'])){
       ...
       if(verifyEmail($_SESSION['userdata']['email'])){
        header('location:../../');
           ...
           if(!$_POST['code']){
            $response['msg']='Ingresa código de 6 dígitos.';
...
if(isset($_GET['forgotpassword'])){
    if(!$_POST['email']){
        $response['msg']="Ingresa tu correo electrónico.";
        $response['field']='email';
        $_SESSION['error']=$response;
        header('location:../../?forgotpassword');

    }elseif(!isEmailRegistered($_POST['email'])){
        $response['msg']="Este correo no se encuentra registrado.";
        $response['field']='email';
        $_SESSION['error']=$response;
        header('location:../../?forgotpassword');
          ...
          $_SESSION['forgot_email']=$_POST['email'];
           $_SESSION['forgot_code']=$code = rand(111111,999999);
            sendCode($_POST['email'],'¿Olvidaste tu clave?',$code);
...
if(isset($_GET['changepassword'])){
    if(!$_POST['password']){
        $response['msg']="Ingresa tu nueva contraseña.";
